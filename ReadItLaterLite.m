...
-(void)sendRequest:(NSString *)newMethod username:(NSString *)username password:(NSString *)password params:(NSString *)additionalParams {
	...
	NSMutableString *params = [NSMutableString stringWithFormat:@"apikey=%@&username=%@&password=%@", [ShareFactory shared].apiKey, [ReadItLater encode:username], [ReadItLater encode:password]];
	...
	[request setValue:[ShareFactory shared].nameOfYourApp!=nil?[ShareFactory shared].nameOfYourApp:@"RIL API iPhone Library v2.0" forHTTPHeaderField:@"User-Agent"];
